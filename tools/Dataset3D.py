...
def getRandomIndices(tiler: UnetTiler3D, n_samples: int) -> list:
    """
    ...
    """
    assert n_samples < len(tiler), \
        'Cannot sample more than {} samples from this tiling'.format(
            len(tiler))
    sample_indices = np.random.choice(np.arange(len(tiler)),
                                      size=n_samples,
                                      replace=False)  # choose n_samples random chunks from the volume
    return sample_indices


def getMeanSignalStrengths(tiler: UnetTiler3D, indices: list) -> list:
    """
    ...
    """
    mean_signal_strengths = [np.mean(tiler.getSlice(i, outputOnly=True))
                             for i in tqdm(indices)]
    return mean_signal_strengths


def sampleMaskProportion(tiler: UnetTiler3D, indices: list) -> list:
    """
    ...
    """
    # number of pixels in the sample volume
    sample_volume = np.prod(tiler.tiling.output_shape)
...
def thresholdedSampling(indices: list, scalar_measure: list, threshold: float, n_samples: int, above_threshold_ratio=0.5) -> list:
    ...
    proba = np.multiply(is_high, above_threshold_ratio/np.sum(is_high)) + np.multiply(
        np.subtract(1, is_high), (1-above_threshold_ratio)/(len(is_high)-np.sum(is_high)))
    # print(proba)
...
class Dataset():
    ...
    def __init__(self, dataset_path, append=True, readonly=False):
        """
        Instantiate a Dataset class linked to
        the repository specified in the dataset path
        ...
        if append:
            ...
            self.dataset_h5 = h5py.File(dataset_path, mode='r')
        else:
            # create new, fail if exists
            self.dataset_h5 = h5py.File(dataset_path, mode='x')
        # keep track of existing groups
        if len(self.keys()) > 0:
            print('Opened dataset with {} preexisting items.'.format(
                len(self.keys())))
            if not readonly:
    ...
    def __getitem__(self, index):
        """
        Returns the i-th element of the dataset. Note that the hdf5 file implements a dictionary
        without guarantees on the ordering of it's keys.
    ...
    def add_tiles(self, tiler, indices, key_prefix='', cropMask=False, preprocessingFunction=None, binarizeMask=False, metadata={}):
        ...
        for index in tqdm(indices, desc='Tiles added'):
            ...
            # fetch the data
            image, mask = tiler.getSlice(
                index), tiler.getMaskSlice(index, cropped=cropMask)
