...
from .deformation import (applyDisplacementField3D,
                          displacementGridField3D)
from .affine import (applyAffineTransformation,
                     getRandomAffine)
...
def elasticDeformation(image, mask):
    ...
    displacementField = displacementGridField3D(image.shape)
    # apply displacement fields
    image = applyDisplacementField3D(image, *displacementField, interpolation_order=1)
    mask = applyDisplacementField3D(mask, *displacementField, interpolation_order=0)
    return image, mask


def affineTransformation(image, mask):
    tm = getRandomAffine()
    image = applyAffineTransformation(image, tm, interpolation_order = 1)
    mask = applyAffineTransformation(mask, tm, interpolation_order=0)
    return image, mask
...
def generateVariants(images, masks, variants,  elasticDeformation=True, affineTransform=True):
    ...
    while len(image_variants) < variants:
        ...
        if elasticDeformation:
            ...
            displacementField = displacementGridField3D(image_shape=image_shape)
        ...
        if affineTransform:
            transformationMatrix = getRandomAffine()
        ...
        for im, mask in zip(images,masks):
            ...
            if len(image_variants) < variants:
                if elasticDeformation:
                    im = applyDisplacementField3D(im, *displacementField, interpolation_order = 1)
                    if image_shape == mask_shape:
                        mask = applyDisplacementField3D(mask, *displacementField, interpolation_order = 0)
                    else:
                        mask = applyDisplacementField3D(mask, *mask_displacementField, interpolation_order= 0)

                if affineTransform:
                    im = applyAffineTransformation(im, transformationMatrix, interpolation_order = 1)
                    mask = applyAffineTransformation(mask, transformationMatrix, interpolation_order=0)
